    ...
    <div className="mt-8 space-y-8">
      <div>
        <h2 className="flex items-center gap-2 text-lg font-semibold">
          <Wallet size={18} className="text-teal-600" /> Assinaturas Pix
        </h2>

        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          <div className="rounded-2xl border border-line bg-white p-5">
            <h3 className="text-sm font-semibold">Configuração Pix da plataforma</h3>
            ...
            <div className="mt-4 space-y-3">
              <input
                ...
                className="w-full rounded-xl border border-line px-4 py-2.5 text-sm outline-none focus:border-teal-600"
              />
          ...
          <div className="rounded-2xl border border-line bg-white p-5">
            <h3 className="text-sm font-semibold">Pagamentos pendentes ({pendentes.length})</h3>
            {carregando ? (
              <p className="mt-4 flex items-center gap-2 text-sm text-ink-soft">
                <Loader2 size={16} className="animate-spin" /> Carregando...
              </p>
            ) : pendentes.length === 0 ? (
              <p className="mt-4 text-sm text-ink-soft">Nenhum pagamento aguardando confirmação.</p>
            ) : (
                ...
                {pendentes.map((p) => (
                  <li key={p.id} className="flex items-center justify-between gap-3 py-3">
                    ...
                    <button
                      ...
                      className="shrink-0 rounded-xl bg-emerald-600 px-4 py-2 text-xs font-semibold text-white transition-all hover:bg-emerald-700 disabled:opacity-50"
                    >
              ...
              <tbody>
                {historico.map((p) => (
                  <tr key={p.id} className="border-b border-line last:border-0">
                    <td className="px-4 py-3 text-sm">{p.profissionais?.nome || "—"}</td>
                    <td className="px-4 py-3 text-sm">{PLANOS[p.plano as keyof typeof PLANOS]?.nome || p.plano}</td>
                    <td className="px-4 py-3 text-sm capitalize">{p.frequencia}</td>
                    <td className="px-4 py-3 text-sm">R$ {p.valor.toFixed(2).replace(".", ",")}</td>
